...
$(document).ready(function(){
    ...
    //查询房东的订单  只有'房东'可以发布房子, 所以url写死了
    $.get("/api/v1.0/user/orders?role=landlord", function(resp){
        if (resp.errno == "0") {
            var orders = resp.data.orders;
            var html_list  = template("orders-list-tmpl", {orders:orders});  //创建
            $(".orders-list").html(html_list);                                     //填充

            // 接单按钮
            $(".order-accept").on("click", function(){
                var orderId = $(this).parents("li").attr("order-id");
                $(".modal-accept").attr("order-id", orderId);  //点击接单后, 再次弹出框 确认 '确定接单'
            });

            // 接单处理
            $(".modal-accept").on("click", function(){
                var orderId = $(this).attr("order-id");  //提取上面写入的 orderId
                $.ajax({
