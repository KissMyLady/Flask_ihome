# 文件名: Flask_Project_Code -> Send_REST
# 创建时间: 2020/6/3 19:35
# 文件名: Flask_Project_Code -> send_msg
# 创建时间: 2020/6/3 8:21
...
class REST:
	...
	def landingCall(self, to, mediaName, mediaTxt, displayNum, playTimes, respUrl, userData, maxCallTime, speed,
	                volume,
	                pitch, bgsound):
		...
		body = '''<?xml version="1.0" encoding="utf-8"?><LandingCall>\
            ...
            <playTimes>%s</playTimes><respUrl>%s</respUrl><userData>%s</userData><maxCallTime>%s</maxCallTime><speed
            >%s</speed>
	...
	def voiceVerify(self, verifyCode, playTimes, to, displayNum, respUrl, lang, userData):
		...
		if self.BodyType == 'json':
			# if this model is Json ..then do next code
			body = '''{"appId": "%s", "verifyCode": "%s","playTimes": "%s","to": "%s","respUrl": "%s","displayNum":
			"%s","lang": "%s","userData": "%s"}''' % (
				self.AppId, verifyCode, playTimes, to, respUrl, displayNum, lang, userData)
		req.data = body
		data = ''
